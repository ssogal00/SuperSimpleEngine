...
// Run after GaussianBlurH.cs.  Reads from the horizontal output (t0)
// and writes to the final blurred texture (u0).
...
void CSMain(uint3 DTid : SV_DispatchThreadID)
{
    ...
    {
        int sampleY = clamp(coord.y + offset, 0, gHeight - 1);
        float w = exp(-(float)(offset * offset) * invTwoSigmaSq);
        result    += InputTex[int2(coord.x, sampleY)] * w;
        weightSum += w;
    }
